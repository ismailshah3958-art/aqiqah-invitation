/**
 * Invitation — Edit your details here
 */
const INVITATION = {
  pageTitle: "Aqiqah & Bismillah — Invitation",

  bismillah: {
    arabic: "بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ",
    english: "In The Name Of Allah, The Most Gracious, The Most Merciful",
  },

  intro: {
    heading: "We are Blessed",
    subtext: "to humbly invite you to the celebration of",
  },

  aqeeqah: {
    label: "A Q I Q A H",
    nameArabic: "محمد شاهنشاه طارق",
    nameEnglish: "Muhammad Shahenshah Tariq",
    relation: "Our Newborn Son",
  },

  bismillahOf: {
    label: "B I S M I L L A H  O F",
    nameArabic: "آئلن طارق",
    nameEnglish: "Aylin Tariq",
    relation: "Our Beloved Daughter",
  },

  message: "Your blessed presence and duas are the greatest gift for our children.",

  event: {
    date: "Sunday, 14th June 2026",
    time: "7:00 PM — 11:00 PM",
    venue: "LuckyOne Apartments Recreation",
    warmRegards: "Zainab and Tariq",
    mapLink: "https://www.google.com/maps/search/?api=1&query=LuckyOne+Apartments+Recreation+Karachi",
  },

  footer: "We sincerely request the honour of your presence",

  quranAyah: {
    arabic: "رَبِّ هَبْ لِي مِن لَّدُنكَ ذُرِّيَّةً طَيِّبَةً",
    reference: "Surah Al-Imran — Ayat 3:38",
    reciter: "Mishary Rashid Alafasy",
    audioUrl: "https://everyayah.com/data/Alafasy_128kbps/003038.mp3",
    volume: 0.3,
    loop: true,
  },
};
